...
with open(args.header, mode="w") as header:
    ...
    namespace pancake::resources {{
      extern const char* {input_id};
    }}
    #endif
    """), file=header, end="")

with open(args.input, mode="r") as infile, open(args.source, mode="w") as source:
    print(dedent(f"""\
    // {args.input.name}
    ...
    for line in infile:
        print(f"\"{escape(line)}\"", file=source)
    print(dedent(f"""\
      ;
    }}
    """), file=source, end="")
